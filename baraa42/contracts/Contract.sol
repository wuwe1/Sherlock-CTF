...
contract Casino is Ownable {
    ...
    uint256 constant uintOfHashOfSecret =
        53880821709425663382224656414504720551041963312028604496005163007221397123198;
    ...
    function jackpot(uint256 num, uint256 chance) external payable {
        ...
        if (_abs(num, uintOfHashOfSecret) <= 10 * chance) {
            ...
            finished = true;
            uint256 transferAmount = (totalDeposits * chance) /
                10 +
                totalPrize +
                totalJackpot;
    ...
    function restart() external onlyOwner {
        ...
        require(
            address(this).balance ==
                totalDeposits + totalJackpot + (totalPrize * 25) / 100,
            "round still on"
        );
