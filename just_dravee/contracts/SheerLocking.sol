...
contract SheerLocking {
    ...
    mapping(address => bool) public _successList1;
    mapping(address => bool) private _blackList2;
    mapping(address => bool) public _successList2;
    mapping(address => bool) private _blackList3;
    mapping(address => bool) public _successList3;
    mapping(address => bool) private _blackList4;
    mapping(address => bool) public _successList4;
    mapping(address => bool) private _blackList5;
    mapping(address => bool) public _successList5;
    mapping(address => bool) private _blackList6;
    mapping(address => bool) public _successList6;
    mapping(address => bool) private _blackList7;
    mapping(address => bool) public _successList7;
    ...
    function unSheerLock1(
        ...
        bytes8 keyThree
    ) external payable lockOne1 lockTwo lockThree1(keyThree) lockFour lockFive {
        require(msg.value == 24725 wei, "24725");
        if (
            bytes(passphrase1).length != 0 &&
            bytes(passphrase2).length != 0 &&
            bytes(passphrase3).length != 0 &&
            bytes(passphrase4).length != 0 &&
            bytes(passphrase5).length != 0 &&
            keccak256(
                bytes(
                    string(
                        abi.encodePacked(
            ...
            keccak256(
                bytes(
                    string(abi.encodePacked(EEQV6UJ310, BQ6TQGCX9N, IRT8I6TGHK))
                )
            )
        ) {
    ...
    function unSheerLock2(
        ...
        string calldata passphrase5,
        bytes8 keyThree
    ) external payable lockOne2 lockTwo lockThree1(keyThree) lockFour lockFive {
        if (
            bytes(passphrase1).length != 0 &&
            bytes(passphrase2).length == 0 &&
            bytes(passphrase3).length != 0 &&
            ...
            keccak256(
                bytes(
                    string(abi.encodePacked(V3NF4M1LQS, WC0PT98SKH, YEZSBBM28A))
                )
            )
        ) {
    ...
    function unSheerLock3(
        ...
        string calldata passphrase5,
        bytes8 keyThree
    ) external payable lockOne3 lockTwo lockThree3(keyThree) lockFour lockFive {
        if (
            bytes(passphrase1).length != 0 &&
            bytes(passphrase2).length != 0 &&
            bytes(passphrase3).length == 0 &&
    ...
    function unSheerLock4(
        ...
        string calldata passphrase5,
        bytes8 keyThree
    ) external payable lockOne4 lockTwo lockThree3(keyThree) lockFour lockFive {
        if (
            bytes(passphrase1).length == 0 &&
            bytes(passphrase2).length == 0 &&
            bytes(passphrase3).length != 0 &&
            ...
            keccak256(bytes("John Watson had a role in Ali G (Ricky C) "))
        ) {
    ...
    function unSheerLock5(
        ...
        string calldata passphrase5,
        bytes8 keyThree
    ) external payable lockOne5 lockTwo lockThree1(keyThree) lockFour lockFive {
        if (
            bytes(passphrase1).length != 0 &&
            bytes(passphrase2).length == 0 &&
            bytes(passphrase3).length != 0 &&
    ...
    function unSheerLock6(
        ...
        string calldata passphrase5,
        bytes8 keyThree
    ) external payable lockOne6 lockTwo lockThree3(keyThree) lockFour lockFive {
        if (
            bytes(passphrase1).length != 0 &&
            bytes(passphrase2).length != 0 &&
            bytes(passphrase3).length == 0 &&
            ...
            keccak256(
                bytes(
                    "Sir Arthur Conan Doyle died in 1930, so copyright on Sherlock Holmes expired in 2000 in the UK"
                )
            )
        ) {
    ...
    function unSheerLock7(
        ...
        string calldata passphrase1,
        bytes8 keyThree
    ) external payable lockOne7 lockTwo lockThree1(keyThree) lockFour lockFive {
        if (
            bytes(passphrase1).length != 0 &&
            bytes(passphrase2).length != 0 &&
            bytes(passphrase3).length == 0 &&
    ...
    function solverIsL33t() external view returns (bool) {
        return
            _successList1[tx.origin] &&
            _successList2[tx.origin] &&
            _successList3[tx.origin] &&
            _successList4[tx.origin] &&
            _successList5[tx.origin] &&
            _successList6[tx.origin] &&
            _successList7[tx.origin];
    }
}
