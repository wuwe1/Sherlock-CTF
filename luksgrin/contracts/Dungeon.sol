...
contract HauntedDungeon {
    ...
    function increaseTreasure() public payable {
        require(
            msg.sender == owner,
            "Only the owner may increase the Treasure"
        );
        treasure += msg.value;
    }
    ...
    modifier alive() {
        ...
        if (lives[msg.sender] <= 0) {
            ...
            emit Talk(
                msg.sender,
                "You just died. Enter the Dungeon again if you dare"
            );
        } else {
    ...
    modifier notCombat() {
        require(
            combatting[msg.sender] == false,
            "You can't do that! You're in the middle of a combat!"
        );
        _;
    }
    ...
    function enterDungeon() public payable existsTreasure {
        require(
            msg.value >= 0.1 ether,
            "You need to pay at least 0.1 ether to enter the Haunted Dungeon"
        );
        treasure += msg.value;
        insideDungeon[msg.sender] = true;
        combatting[msg.sender] = false;
        ...
        emit Talk(
            msg.sender,
            "Welcome to the Haunted Dungeon. Will you be able to conquer it?"
        );
    }

    // Sets up the Dungeon's store
    function store(string memory _prod)
        public
        payable
        existsTreasure
        checkInsideDungeon
    {
        require(
            prices[_prod] == msg.value,
            "Incorrect amount of funds provided to buy that product"
        );
        require(
            storeAccessLimit[msg.sender] > 0,
            "You entered the store too many times! Store is closed now"
        );
    ...
    function enterFloor() private existsTreasure notCombat returns (bool) {
        ...
        if (currentFloor[msg.sender] > 5) {
            ...
            emit Talk(
                msg.sender,
                "Congratulations!  You conquered the Dungeon and took the treasure"
            );
            insideDungeon[msg.sender] = false;
            storeAccessLimit[msg.sender] = 3;
            currentFloor[msg.sender] = 0;
            ...
            currentMnstr[msg.sender] = [
                (int256(random()) % 10) +
                    baseStats[currentFloor[msg.sender] - 1],
                2 *
                    (int256(random()) % 7) +
                    baseStats[currentFloor[msg.sender] - 1]
            ];
    ...
    function turn() public existsTreasure checkInsideDungeon alive {
        ...
        if ((attack[msg.sender] >= currentMnstr[msg.sender][0]) && (!victory)) {
            emit Talk(
                msg.sender,
                "You killed the monster! You recover some of the money spent"
            );
            ...
            (defense[msg.sender] >= currentMnstr[msg.sender][1]) && (!victory)
        ) {
            currentMnstr[msg.sender][0] -= attack[msg.sender];
            defense[msg.sender] -= currentMnstr[msg.sender][1];
        } else if (!victory) {
            lives[msg.sender] -= (currentMnstr[msg.sender][1] -
                defense[msg.sender]);
        } else {
            victory = false;
        }
