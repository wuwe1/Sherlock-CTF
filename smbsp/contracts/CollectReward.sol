...
contract CollectReward {
        ...
        returns (uint256 withdrawTime, uint256 amount)
    {
        ...
        uint256 lastInterval = _calculateTimestamp(currentTime); // => programStartTime + (8 * ONE_DAY)

        withdrawTime = programStartTime;

        if (lastInterval <= withdrawTime) return (0, 0); // not gonna happend
        if (_startTime > 0) {
            withdrawTime = _calculateTimestamp(_startTime);
        }
        ...
        // i must contains {programStartTime  + 518400}
        // duration > withdrawTime + 6 * ONE_DAY
