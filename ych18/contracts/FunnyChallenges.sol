...
contract FunnyChallenges {
    bytes32 constant magic_hash =
        0x841261bf11a58fbabcbcb7d4efd531da55121171dea830574030238956ac2eed;
    bool public first_challenge_solved;
    ...
    function DontGiveUp(string calldata ch1, string calldata ch2) external {
        require(
            keccak256(abi.encodePacked(ch2)) !=
                keccak256(abi.encodePacked("CTF")),
            "NO Try again"
        );
        bytes memory ch2_bytes = bytes(ch2);
        //   != [C, T, F]
        require(
            uint8(ch2_bytes[0]) != 84 &&
                uint8(ch2_bytes[0]) != 70 &&
                uint8(ch2_bytes[0]) != 67,
            "Nice Try but NOOOO!"
        );

        require(
            ch2_bytes.length <= 3,
            "You think of brute force? not a good idea"
        );
    ...
    function transfer(address to, uint256 magic_number) public payable {
        ...
        require(
            msg.sender == address(this) || hash_magic_number == magic_hash,
